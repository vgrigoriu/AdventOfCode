object TreeHouse extends IntPuzzle:
    def solve(): Int =
        val input = readInput()
        val forest = toDigitArray(input)
        countVisibleTrees(forest)

object TreeHouse2 extends IntPuzzle:
    def solve(): Int =
        val input = readInput()
        val forest = toDigitArray(input)
...
private def isVisible(forest: Vector[Vector[Int]], row: Int, col: Int): Boolean =
    val treeHeight = forest(row)(col)
    val visibleFromWest = (0 until col).forall(c => forest(row)(c) < treeHeight)
    val visibleFromEast = (col + 1 until forest(row).length).forall(c => forest(row)(c) < treeHeight)
    val visibleFromNorth = (0 until row).forall(r => forest(r)(col) < treeHeight)
    val visibleFromSouth = (row + 1 until forest.length).forall(r => forest(r)(col) < treeHeight)
...
private def scenicScore(forest: Vector[Vector[Int]], row: Int, col: Int): Int =
    ...
    val blocksViewWest = (0 until col).reverse.find(c => treeHeight <= forest(row)(c))
    val scoreWest = blocksViewWest match
        case None => col
        case Some(c) => col - c
    
    val blocksViewEast = (col + 1 until forest(row).length).find(c => treeHeight <= forest(row)(c))
    val scoreEast = blocksViewEast match
        case None => forest(row).length - 1 - col
        case Some(c) => c - col
    
    val blocksViewNorth = (0 until row).reverse.find(r => treeHeight <= forest(r)(col))
    val scoreNorth = blocksViewNorth match
        case None => row
        case Some(r) => row - r
    
    val blocksViewSouth = (row + 1 until forest.length).find(r => treeHeight <= forest(r)(col))
